...
import mm from "../images/logo header.png";
import { useState, useEffect } from "react";

export default function Header({el}:{el:string}) {
  const [activeNav, setActiveNav] = useState(el);

  const navItems = ["Accueil", "A propos", "Services", "Contacte"];

  // Listen to global navigation events (page:navigate) to update active nav
  useEffect(() => {
    function handlePageNavigate(e: Event) {
      const ev = e as CustomEvent;
      const page = ev?.detail?.page;
      if (typeof page === "string") setActiveNav(page);
    }

    window.addEventListener(
      "page:navigate",
      handlePageNavigate as EventListener
    );
    return () =>
    ...
    <header className="fixed top-6 left-0 right-0 z-50 pointer-events-none">
      <nav className="pointer-events-auto flex items-center justify-between px-8 py-3 max-w-[95%] mx-auto ">
        ...
        <div className="flex items-center gap-1 bg-[#2a2a2a]/50 backdrop-blur-sm rounded-full px-4 py-2.5 border border-white/5">
          {navItems.map((item) => (
            <Link key={item} to={`${item === 'Accueil' ? '/' : '/aprp'}`}>
              <button
                onClick={() => {
                  setActiveNav(item);
                  window.dispatchEvent(
                    new CustomEvent("page:navigate", { detail: { page: item } })
                  );
                }}
                className={`px-6 py-2 rounded-full font-light tracking-wide transition-all duration-300 text-[15px] ${
                  activeNav === item
